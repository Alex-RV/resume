...
export default async function getAuthInfoPopUp(windowObj: Window): Promise<AuthInfo | null> {
  ...
  return new Promise<AuthInfo | null>((resolve) => {
    ...
    const authUrl = `https://accounts.google.com/o/oauth2/auth?client_id=${clientId}&redirect_uri=${redirectUri}&response_type=code&scope=https://www.googleapis.com/auth/calendar&access_type=offline&prompt=consent`;
